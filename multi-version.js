...
function createJsIndex(name, versions) {
  ...
  versions.forEach((v) => {
    ...
    var compDir = path.join(packageDirectory, `${strippedName}`, `${v}`, `${strippedName}.js`);
    if ( require('os').platform() == 'win32' ) {
      compDir = compDir.split('\\').join('\\\\');
    }
...
function createScssIndex(name, versions) {
  ...
  versions.forEach((v) => {
    var cssPath = path.resolve(packageDirectory, strippedName, v, `${strippedName}.css`)
    if ( require('os').platform() == 'win32' ) {
      cssPath = cssPath.split('\\').join('\\\\');
    }
    //imports
