...
const cedarPackages = require(path.join(__dirname, 'cedar-packages.json'));

const packageDirectory = path.join(__dirname, `cedar-packages`);
...
function downloadPackages(packageName, versions) {
  versions.forEach((version) => {
    ...
    const versionDir = path.join(packageDirectory, `${packageName.split('/')[1]}`, `${version}`);
    downloadPackage({
      arg: `${packageName}@${version}`,
      dir: versionDir,
    }).then(() => {
      // installed packages have a lot of files we don't care about, just move files from the dist folder
      fs.moveSync(path.join(versionDir, packageName, `dist`), versionDir);
      fs.removeSync(path.join(versionDir, `@rei`));
    });
  });
}
...
function createJsIndex(name, versions) {
  ...
  versions.forEach((v) => {
    ...
    const compDir = path.join(packageDirectory, `${strippedName}`, `${v}`, `${strippedName}.js`);
...
export default {
  ...
  //output
  fs.outputFileSync(path.join(packageDirectory, strippedName, 'index.js'), contents);
}
...
function createScssIndex(name, versions) {
  ...
  versions.forEach((v) => {
    const cssPath = path.join(packageDirectory, strippedName, v, `${strippedName}.css`)
    //imports
    contents += `@import '${cssPath}';${endOfLine}`;
  });

  //output
  fs.outputFileSync(path.join(packageDirectory, strippedName, 'index.scss'), contents);
}
